...
func handleReadMarkdownFileResource(ctx context.Context, req mcp.ReadResourceRequest) ([]mcp.ResourceContents, error) {
	...
	// Extract filename from template parameters (new format: markdown://{filename})
	filename := ""
	...
	if filename == "" && strings.HasPrefix(req.Params.URI, "markdown://") {
		filename = strings.TrimPrefix(req.Params.URI, "markdown://")
	}

	// Fallback: Parse URI to extract filename parameter (backward compatibility)
	if filename == "" && strings.Contains(req.Params.URI, "?filename=") {
		parts := strings.Split(req.Params.URI, "?filename=")
		if len(parts) > 1 {
			filename = parts[1]
		}
	}
